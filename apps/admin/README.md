...
- **Cena po osobi vs po jedinici nije uporediva** (javni sajt, Korak 3, za odluku) — PDF cenovnik ima cenu po osobi (autobuski prevoz) i cenu za smeštajnu jedinicu (sopstveni prevoz), a razlika je samo u tekstu `naziv`. Sortiranje "najjeftinije prvo" meša jabuke i kruške čim se odobre ponude po jedinici (u dev bazi 76 od 86 na čekanju). Predlog: kolona `cena_tip` (`po_osobi`/`po_jedinici`) iz AI ekstrakcije + izvedena cena po osobi (jedinica / `max_gostiju`). Detalji u `docs/PLAN-JAVNI-SAJT.md`, Korak 3.
- **Pretraga sajta ne razume dijakritike i ne gleda `naziv`** (javni sajt, Korak 3) — "sid" ne nalazi "Šid", "cacak" ne nalazi "Čačak"; slobodan tekst traži samo u `destinacija`, ne i u nazivu vile/apartmana ("Estia" ne daje ništa). Rešenje: `unaccent` (ili normalizovana kolona) i pretraga i po `naziv`.
...
- [x] **Korak 1 — RLS politika za javno čitanje.** Migracija `20260920100000_public_offers_select`: nova `offers_select_public` policy, `FOR SELECT TO anon USING (status = 'published')`. Testirano direktno na dev bazi (simulacija `anon` role bez JWT-a) — anon vidi SAMO `published` ponude, nema pristup `uploads`/`clicks`, postojeće politike za staff/agencije nepromenjene. Sama politika je red-level; kolone su ograničene naknadnim grantom (vidi ispod).
  - **Column-level grant na `offers`** (nalaz iz Koraka 2): migracija `20260920120000_public_offers_column_grant` — `anon` čita samo 11 kolona (`id`, `agency_id`, `naziv`, `destinacija`, `datum_polaska`, `datum_povratka`, `cena_eur`, `max_gostiju`, `dostupno_mesta`, `status`, `updated_at`). `upload_id`, `confidence_score`, `published_at`, `created_at` i `kontakt_url` daju `permission denied` (i za `select *`, filter i sortiranje), čak i direktnim PostgREST pozivom. `kontakt_url` je skriven da se `/go/` brojač klikova ne može zaobići; `/go/` ga čita preko `service_role`. Nova kolona na `offers` nije javna dok se ne doda u grant novom migracijom.
